...
function RequestCloudboxDownload(type, id, rev)
	print("[Cloudbox] Requested download: type=" .. tostring(type) .. " id=" .. tostring(id) .. " rev=" .. tostring(rev))
...
net.Receive("CloudboxServerDownloadProgress", function()
	...
	if !ActiveCloudboxDownloads[id] then return end // guard: a race with the download-finished message can already have cleared this
...
net.Receive("CloudboxServerDownloadFinished", function()
	...
	if !ActiveCloudboxDownloads[id] then return end // guard: a race with the progress message can already have cleared this

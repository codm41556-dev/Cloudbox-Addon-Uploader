...
function RegisterCloudboxDownload(id, rev, requester, isInclude)
	...
	local block = hook.Run("RegisterCloudboxDownload", id, rev, requester)
	...
	if file.Exists(path, "DATA") then // if we have the package info locally then load it
		...
		local url = GetConVar("cloudbox_api_url"):GetString() .. "/packages/get?id=" .. id .. "&rev=" .. rev

		print("[Cloudbox] Fetching package info from " .. url)

		http.Fetch(url, function(body, size)
			if size == 0 then
				print("[Cloudbox] Fetch succeeded but response was empty: " .. url)
				return
			end
			...
			print("[Cloudbox] Failed to fetch package info from " .. url .. ": " .. tostring(err))
		end)
	end
end

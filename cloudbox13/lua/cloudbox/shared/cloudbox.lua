...
// lets a self-hosted / locally tested backend override the hardcoded
// production API host. Replicated so a server pointing at its own backend
// (e.g. during local testing) also redirects its clients there.
CreateConVar("cloudbox_api_url", "https://api.cl0udb0x.com", {FCVAR_ARCHIVE, FCVAR_REPLICATED}, "Cloudbox backend API URL")
...
function ExecuteCloudboxPackage(info)
	...
	elseif info["type"] == "addon" then
		// modern, folder-based addon package (lua/autorun, entities, weapons,
		// effects, materials, models, sound, etc) - see addonloader.lua.
		// unlike entity/weapon packages there's no single embedded script to
		// RunString: everything comes from the mounted GMA's own file layout.
		LoadCloudboxAddon(info)
	end
	...
	if CLIENT then
		...
		if ActiveCloudboxDownloads[info["id"]] then // guard: a race with the progress-notification message can already have cleared this
...
function MountCloudboxPackage(info, attempt)
	...
	local url = GetConVar("cloudbox_api_url"):GetString() .. "/packages/getgma?id=" .. info["id"] .. "&rev=" .. info["rev"]

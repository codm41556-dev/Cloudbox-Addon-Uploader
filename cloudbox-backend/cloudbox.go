...
import (
	...
	"github.com/flatgrassdotnet/cloudbox/api/addons"
...
func main() {
	...
	s3endpoint := flag.String("s3endpoint", "", "custom s3-compatible endpoint (e.g. a local MinIO instance for testing); leave empty for normal AWS S3")
	...
	err := db.Init(*dbuser, *dbpass, *dbproto, *dbaddr, *dbname, *s3endpoint)
	if err != nil {
	...
	http.HandleFunc("GET /auth/steamlogin", auth.SteamLoginStart)
	http.HandleFunc("GET /auth/steamlogin/callback", auth.SteamLoginCallback)
	...
	http.HandleFunc("POST /addons/upload", addons.Upload)
	http.HandleFunc("OPTIONS /addons/upload", addons.Upload)
	http.HandleFunc("POST /addons/delete", addons.Delete)
	http.HandleFunc("OPTIONS /addons/delete", addons.Delete)

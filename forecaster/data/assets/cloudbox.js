...
// --- In-game: "Delete" option on the right-click menu for addons you own ---
// Only runs in-game (the html.gm13 class is set by the forecaster when the
// GMod user agent is detected). It asks the game (via the cloudbox.GetSteamID
// Lua bridge) for the local player's SteamID64, then for every addon item
// whose stored author matches, it reveals the hidden "Delete" entry in that
// item's right-click context menu and wires its click to the
// cloudbox.DeleteAddon Lua bridge (which shows an in-game confirm dialog and
// then opens the authenticated upload page to actually perform the delete -
// reusing the existing, proven Steam-OpenID ownership check rather than a
// weaker in-game auth path).
if (document.documentElement.classList.contains("gm13")) {
        try {
                cloudbox.GetSteamID(function (sid) {
                        if (!sid) { return; }
                        var owned = document.querySelectorAll(".item[data-type='addon'][data-author='" + sid + "']");
                        for (var i = 0; i < owned.length; i++) {
                                (function (el) {
                                        var lnk = el.querySelector("dialog.contextmenu a.delete-link");
                                        if (!lnk) { return; }
                                        ...
                                        lnk.addEventListener("click", function (e) {
                                                ...
                                                if (cloudbox.DeleteAddon) {
                                                        cloudbox.DeleteAddon(id, name);
                                                }
                                                var menu = el.querySelector("dialog.contextmenu");
                ...
                // cloudbox bridge unavailable (e.g. desktop preview) - no-op
        }
}